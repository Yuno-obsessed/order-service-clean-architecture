...
public class AddOrderProductCommand {
    ...
    public AddedOrderProductDTO handle(AddOrderProductCommandDTO orderCommandDTO) {
        ...
        Order order = orderReader.getOrderById(orderCommandDTO.orderID);
        ...
        String imageName = productImages.getImageNames().stream().filter(e -> e.lastIndexOf('0') == e.length() - 1)
                    .findFirst()
                    .orElse(null);
        String imageURL = fileStorage.getFileURL(imageName, productImages.getBucketName());
